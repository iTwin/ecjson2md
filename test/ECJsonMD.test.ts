...
describe("ECJsonToMD", () => {
  ...
  describe("Schema name, description, alert, and frontmatter", () => {
    ...
    before(async () => {
      ...
      await generator.generate(schemaFilePath, outputFilePath, true);
    ...
    it("should generate the alert properly", async () => {
      lines = markdownText.split("\n");
      assert.isTrue(lines.indexOf("[!alert text=\"This documentation represents a nonreleased version of this schema\" kind=\"warning\"]") > -1, "The nonrelease schema alert was not written out");
    });
  ...
  describe("Front matter, schema title, and schema description structure", () => {
    let schemaFilePath: string;
    let outputFilePath: string;
    let generator: ECJsonMarkdownGenerator;
    let searchDirs: string[];
  ...
  // This section tests that the schema information is in the correct order
  describe("Front matter, schema title, and schema description structure with nonrelease flag", () => {
    let schemaFilePath: string;
    let outputFilePath: string;
    let generator: ECJsonMarkdownGenerator;
    let searchDirs: string[];
    ...
    it("nonrelease alert should come after the front matter", () => {
      lines = markdownText.split("---\n")[2].split("\n");

      // Assert that the alert is written following a blink line after the frontmatter
      assert.equal(lines[0], "");
      assert.equal(lines[1], "[!alert text=\"This documentation represents a nonreleased version of this schema\" kind=\"warning\"]");

    });

    it("title of the schema should come after the alert", () => {
      lines = markdownText.split("\n");
      const assertIndex = lines.indexOf("[!alert text=\"This documentation represents a nonreleased version of this schema\" kind=\"warning\"]");
      assert.isTrue((assertIndex >= 0), "cannot find the nonrelease alert");

      // Assert that the title of the schema is written following a blank line after the alert
      assert.equal(lines[assertIndex + 1], "");
      assert.equal(lines[assertIndex + 2], "# BasicClasses");
    });
